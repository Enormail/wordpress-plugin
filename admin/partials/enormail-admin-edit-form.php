...
<div class="enormail-wrapper">
    ...
    <div class="enormail-content enormail-content">
        ...
        <h1><?php echo __( 'Edit form', 'enormail' ); ?></h1>

        <div class="enormail-form-wrapper">
            <?php if ($apiFormList) : ?>
                <form method="post" action="<?php echo admin_url('admin.php?page=enormail-admin&view=edit_form&noheader=true'); ?>">
                    <input type="hidden" name="id" value="<?php echo absint($form->id); ?>" />
                    <table class="form-table">
                        <tr>
                            <th><?php echo __( 'Form title', 'enormail' ); ?></th>
                            <td>
                                <input id="enormail_form_title" name="enormail_form_name" type="text" value="<?php echo esc_html($form->name); ?>" class="regular-text">
                            ...
                            <td>
                                <select id="js-enormail-form-select" name="enormail_formid">
                                    <?php foreach ($apiFormList as $formId => $name) : ?>
                                        <option value="<?php echo  $formId; ?>"<?php if ($formId == $form->formid) : ?> selected<?php endif; ?>><?php echo  $name; ?></option>
                                    <?php endforeach; ?>
                                </select>
                    ...
                    <button type="submit" class="button button-primary button-hero"><?php echo __( 'Save form', 'enormail' ); ?></button>
